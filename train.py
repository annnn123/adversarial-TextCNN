...
def train(config):
    ...
    if config.dev_data_path is not None:
        ...
        test_data = load_data(config.test_data_path)
    print(f"[Dataset Info] name: {config.dataset}, train size: {len(train_data)}, dev size: {len(dev_data)}, test size: {len(test_data)}")
    ...
    if config.adv_method=='PGD':
        ...
        best_model_save_path = config.model_save_dir + f"{config.model_name}_{config.adv_method}_eps{config.eps}_PGD_steps{config.PGD_steps}_best_model.bin"
        ...
        Free_model = Free(model, config.eps)
        best_model_save_path = config.model_save_dir + f"{config.model_name}_{config.adv_method}_eps{config.eps}_Free_num_replays{config.Free_num_replays}_best_model.bin"
        print(f"{config.model_name} using {config.adv_method} adversarial training with eps={config.eps}, Free_num_replays={config.Free_num_replays}.")
    elif config.adv_method=='FGSM':
        FGSM_model = FGSM(model, config.eps, config.alpha)
        best_model_save_path = config.model_save_dir + f"{config.model_name}_{config.adv_method}_eps{config.eps}_best_model.bin"
    ...
    for epoch in range(1, config.max_epoches + 1):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if config.adv_method == 'Free':
                for t in range(config.Free_num_replays):
                    Free_model.attack(emb_name='embedding')
                    model.zero_grad()  # reset gradients

                    tmp_output = model(input_ids)
                    loss = criterion(tmp_output, labels)
                    loss.backward()  # back propagation
                    Free_model.backup_grad()

                    Free_model.restore_emb('embedding')
            ...
            try:
                ...
                report_loss += loss_adv.item()
                cum_loss += loss_adv.item()
            report_examples += 1
            cum_examples += 1
